...
| `kofa_decision_text` | `search_vector` + GIN | `embedding` VECTOR(1536), IVFFlat (lists=100), 100% dekning |
...
| C: Vektorsøk (embedding) | pgvector embeddings + hybrid søk (som lovdata) | Implementert — 139 216 embeddings, IVFFlat-indeks |
...
KOFA-avgjørelsesteksten (139 216 avsnitt) hadde ingen søkefunksjonalitet — kun metadata-FTS på `kofa_cases.fts`. For å la LLM finne relevante avgjørelser basert på innholdet i selve begrunnelsene trengs:
...
**IVFFlat-indeks** bygget 2026-02-11: `kofa_decision_text_embedding_ivfflat_idx` (lists=100, vector_cosine_ops, 1 088 MB).
...
Statistisk analyse av 139 216 avsnitt + stratifiserte stikkprøver (6 saker, 3 epoker) før embedding-generering.
...
- [x] Kjøre `python scripts/embed_kofa.py` — 139 216 avsnitt embeddet (2026-02-11)
- [x] Bygge IVFFlat-indeks — lists=100, vector_cosine_ops, 1 088 MB (2026-02-11)
- [x] Validering av hybrid søk (2026-02-11)

### Validering av hybrid søk (2026-02-11)

Tre testsøk med kvalitativ vurdering av alle treff (seksjon=vurdering, limit=10):

**Test 1: Eksakt juridisk term** — «forpliktelseserklæring»

FTS og hybrid gir tilsvarende resultater. FTS fungerer godt på eksakte termer. Hybrid rerangerer marginalt (2012/6 rykker opp pga. høy semantisk likhet).

**Test 2: Naturlig språk** — «kan oppdragsgiver avvise tilbud som ikke oppfyller kvalifikasjonskrav»

| Kilde | Treff | Relevante | Presisjon |
|---|---|---|---|
| FTS | 10 | ~8 | ~80% |
| Hybrid-eksklusive (fts=0) | 6 | 6 | 100% |

De 6 hybrid-eksklusive treffene brukte synonymer FTS ikke fanger: «kvalifikasjonene» (2005/197), «kvalifikasjonskriterier» (2010/276), «krav satt i konkurransegrunnlaget» (2017/108). Alle inneholdt substansielle rettssetninger om avvisningsplikt/-rett.

**Test 3: Konseptuelt søk** — «tildelingskriterier utenfor det som er relevant for kontrakten»
...
FTS matchet på «tildelingskriter*» + «utenfor» i irrelevante kontekster (tilbudsstruktur, komitémandat, klagers anførsler). Hybrid fant NF-doktrine-formuleringer (2007/92, 2009/171, 2010/116), ulovlige tildelingskriterier (2003/165, 2013/74), og grensedragning kvalifikasjon/tildeling (2009/82, 2015/85) — uten at søketermene forekom direkte.

**Konklusjon:** Hybrid-tilnærmingen validert. Vektorsøk gir størst merverdi på konseptuelle søk der terminologien varierer. FTS-vekt 0.3 bekreftet riktig — FTS-komponenten bidrar på eksakte termer men genererer støy på bredere formuleringer. Se `docs/metode-rettslig-analyse.md` for implikasjoner for søkestrategi.
